...
from models import UNet, basic_SEResUNet
from utils.data_loaders import get_cropimg_training_loaders
...
parser.add_argument('--exp_name', default='single_nuclei_lossless_augmentations', type=str,
...
train_loader, valid_loader, len_train, len_valid = get_cropimg_training_loaders(imsize=args.img_size, 
...
def train():
     ...
     for i, data in enumerate(train_loader):
          ...
          pred_msk = net(imgs)
          loss = msk_crit(pred_msk, msks) + args.lamb * l1_crit(F.sigmoid(pred_msk), msks)
          # optimize network
          loss.backward()
...
def validate():
     ...
     for j, data in enumerate(valid_loader):
          ...
          vpred_msk = net(val_imgs) 
          v_l = msk_crit(vpred_msk, val_msks) + args.lamb * l1_crit(F.sigmoid(vpred_msk), val_msks)
